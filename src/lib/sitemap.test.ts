...
const entries = [...sitemap.matchAll(/<url>\s*<loc>([^<]+)<\/loc>\s*<title>([^<]*)<\/title>/g)].map(
  (match) => ({ loc: match[1], title: match[2] }),
)

describe('public/sitemap.xml', () => {
  it('lists every route once, under its canonical URL and title', () => {
    expect(entries).toEqual(
      ROUTE_META.map((meta) => ({ loc: canonicalUrl(meta.path), title: meta.title })),
    )
  })
